...
File name (Note: Consult ‘core/state/StateManager.json’ to get the current task number from task_counters.tasks.current and update it automatically with validation. Use the current number for the file, then increment it by 1 using the validation rules to avoid conflicts.): [number from ‘core/state/StateManager.json’]-[name]-plan.md
```markdown
