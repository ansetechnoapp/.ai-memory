...
- **File naming convention**: `[number]-[topic]-research.md`
  - Number: Consult `core/state/StateManager.json` to get task_counters.research.current
  - Use the number listed
  - **Automatically increment** the number by 1 using validation rules to avoid conflicts
  - Always find and use the next available number

### 🔢 Numbering System Logic
1. **Read** `core/state/StateManager.json` to get task_counters.research.current
2. **Use** the current number for your file name
3. **Increment** the counter by 1 with automatic validations
4. **Ensure** sequential numbering and conflict avoidance
...
### File Operations
1. **Read** current task number from `core/state/StateManager.json` task_counters.research.current
2. **Generate** file name using format: `[number]-[descriptive-topic]-research.md`
3. **Create** comprehensive research report
4. **Increment** the counter in StateManager.json with validations
