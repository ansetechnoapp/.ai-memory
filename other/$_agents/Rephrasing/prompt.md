...
- File naming: `[number]-[name]-$_rephrase.md`
  - Number: Read from `.ai-memory/$_rephrasing/currentTaskNumber.txt`
  - Use the current number for the file name.
  - Increment the number by 1 and update the file for next use.
  - Always use the next available number in sequence.

### 🔢 Numbering System Steps
1. Read `.ai-memory/$_rephrasing/currentTaskNumber.txt` to get the current number.
