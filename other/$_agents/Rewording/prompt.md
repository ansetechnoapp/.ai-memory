...
- **File naming convention**: `[number]-[name]-$_rephrase.md`
  - Number: Consult `core/state/StateManager.json` to get task_counters.rephrasing.current
  - Use the number listed
  - **Automatically update** the number by adding 1 using validation rules to avoid conflicts
  - Always find and use the next available number

### 🔢 Numbering System Logic
1. **Read** task_counters.rephrasing.current from `core/state/StateManager.json`
2. **Use** the current number for your file name
3. **Update** the counter with the next number (current + 1) following validation rules
4. **Ensure** sequential numbering is maintained with automatic conflict prevention
...
### File Management:
- **Automatic numbering** from StateManager.json
...
### File Operations:
1. **Read** current task number from `core/state/StateManager.json` task_counters.rephrasing.current
2. **Generate** file name using format: `[number]-[descriptive-name]-plan.md`
3. **Create** comprehensive markdown report
4. **Update** the counter in StateManager.json with incremented value using validation
